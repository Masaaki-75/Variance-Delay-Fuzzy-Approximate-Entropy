%% Indices Table
% run 'main.m' to get the mean values and standard deviation
%   of the frequency indices and nonlinear indices.
% or simply
%   load 'NMS_mean';
%   load 'NMS_std';
...
% N & M
for ii = 1:5
    temp_y = [indN(ii,:),indM(ii,:)];
    % 0 for normal, 1 for mmOSA, and 2 for sOSA.
    temp_group = [zeros(size(indN(ii,:))),ones(size(indM(ii,:)))];
    pNM(ii) = anova1(temp_y,temp_group,'off');
end

% N & S
for ii = 1:5
    temp_y = [indN(ii,:),indS(ii,:)];
    % 0 for normal, 1 for mmOSA, and 2 for sOSA.
    temp_group = [zeros(size(indN(ii,:))),2*ones(size(indS(ii,:)))];
    pNS(ii) = anova1(temp_y,temp_group,'off');
end

% M & S
for ii = 1:5
    temp_y = [indM(ii,:),indS(ii,:)];
    % 0 for normal, 1 for mmOSA, and 2 for sOSA.
    temp_group = [ones(size(indM(ii,:))),2*ones(size(indS(ii,:)))];
    pMS(ii) = anova1(temp_y,temp_group,'off');
end
